...
\renewcommand{\QP}{\textsc{Maximum Quadratic Programming}}
...
%%The budget problem for the \emph{minimization} form of the quadratic assignment problem is strongly $\NP$-complete; furthermore, the minimization problem itself is not in $\APX$ \autocite{sg76} and I think it is in fact $\NPO$-complete (since \textsc{Minimum Traveling Salesperson} is, and I think that's a special case).
...
The budget problem for \QP{} is $\NP$-complete \autocite{vavasis90}, so the optimization problem is in $\NPO$.
...
On the other hand, there seems to be a fully polynomial time approximation scheme for \RMA{} \autocite{v14}; in other words, \RMA{} is (I believe) in $\FPTAS$.
...
\RMD{} is a special case of the \CQP{} problem (itself a special case of the minimization form of \QP{}) \todo{reference or explain why this is a special case}.
...
\end{theorem}
\begin{proof}
  Computing an exact solution to \CQP{} can be done in polynomial time \autocite{gl90}.
  Since \textsc{Linear Programming} is a special case of \CQP{}, and \textsc{Linear Programming} is $\P$-complete \autocite{ghr95}, \CQP{} is $\P$-complete as well.
\end{proof}

Since the budget problem for \CQP{} is in $\P$, \RMD{} is in $\PO$.

\todo{Is it in $\NNCO$? Approximable in $\NC$?}
