...
\begin{equation*}
  \agr_\pi(G, H) = \frac{\tr(AP^TBP)}{\max(\|A\|_1, \|B\|_1)}.
\end{equation*}
...
If $\agr(G, H) = 1$, then $G$ is isomorphic to $H$.
...
A second attempt may be something like the zig-zag product.
...
\begin{proof}
  ...
  For the denominator,
  \begin{equation*}
    \max(\|A \otimes A\|_1, \|B \otimes B\|_1) = \max(\|A\|^2_1, \|B\|^2_1) = \max(\|A\|_1, \|B\|_1)^2.
  \end{equation*}
...
\todo{
  ...
  If so, and $X \otimes X = Y \otimes Y$ implies $X = Y$, then we're in a very good position.
}
