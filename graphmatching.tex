...
The \emph{direct sum} of square matrices $A$ and $B$, denoted $A \oplus B$, is the block matrix
\begin{equation*}
  \begin{bmatrix}
    A & O \\
    O & B
...
Relaxing the constraint ``$P \in \{0, 1\}^{n \times n}$'' to the constraint ``$P \geq O$'', in other words, allowing $P$ to be a \emph{doubly stochastic} instead of a permutation matrix, yields the \emph{relaxed graph matching problem}.
...
\begin{definition}[\MD]
  ...
  \begin{tabular}{r p{9.3cm}}
    ...
    Solution: & $n \times n$ Boolean matrix $P$, subject to the constraints $P \1 = P^T \1 = \1$. \\
    Measure: & $\|PA - BP\|^2$. \\
    Type: & minimization.
  \end{tabular}
\end{definition}
...
It is also the special case of the budget problem for \MA{} in which the budget is the number of edges in the graph (in other words, it is the problem of finding a permutation that causes every possible agreement).
